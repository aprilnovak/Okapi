...
The choice between MOON and Okapi for the Master App is more arbitrary than the choice to not choose BISON as the Master App, but is still based on characteristics of the coupling. Because Okapi is the only App that will solve for the entire domain (MOON solves in the fluid, BISON solves in the fuel), it is logical to choose Okapi as the Master App. So, Okapi is selected as the Master App, which both performs its own solve and transfers data between BISON and MOON. The figure below shows all of the necessary methods to perform a three-way coupled simulation, with numbers indicating the order in which they would occur for a single Picard step. BISON is arbitrarily chosen to run first. 
...
\begin{enumerate}
\item Initialize the Master App. The actions performed here are generally the same regardless of which App is the Master App (i.e. this is not the stage at which the OpenMC input XML files would be read), since this purely sets up the entire MOOSE simulation by initializing MPI, allocating storage, etc. This is where initial conditions specified in the master input file are set.
\item Initialize MOON. This calls the {\tt NekExecutioner::init()} method and reads Nek input files to perform internal actions such as setting initial values.
\item Initialize BISON. This calls the initialization method of the executioner specified in the BISON input file.
\item Initialize the Master App. This step is not redundant with step 1, but rather this calls the {\tt init()} method of the {\tt OpenMCExecutioner}, which reads OpenMC input files and performs internal actions such as setting initial values.
\item Execute all transfers on {\tt timestep\_begin} with direction {\tt to\_multiapp}.
	\begin{enumerate}
	\item Transfer heat flux boundary condition coefficients from Okapi to MOON.
	\end{enumerate}
\item Solve Nek.
\item Execute all transfers on {\tt timestep\_begin} with direction {\tt from\_multiapp}.
	\begin{enumerate}
	\item Transfer temperature boundary condition coefficients from MOON to Okapi.
	\end{enumerate}
\item Solve Okapi.
...
And for BISON executing first, the sequence of events are similar, except that steps 5-7 dealing with MOON execution and transfer are swapped with steps 9-11 dealing with BISON execution and transfer.
...
It becomes increasingly difficult as the number of coupled codes increases to ensure consistency between all of the initial conditions and system specifications. To attempt to achieve consistency, all initial conditions should be set from MOOSE. However, if desired, these MOOSE initial conditions can be omitted, and initial conditions set in the usual manner in the code's input files (OpenMC's XML files or Nek's {\tt rea} file). 
...
When MOON runs before BISON, initial conditions must be specified for fields that are inputs to MOON and for those fields that are inputs to Okapi that would have been determined by BISON. This section discusses how to set these initial conditions.

\subsubsection{Fuel Wall Heat Flux and Temperature}
Because BISON does not run before MOON for the very first time step, an initial condition must be set on the fuel wall heat flux. Because this is a MOOSE variable (represented as a scalar auxvariable), it is most sensible to define this parameter directly in the master input file. The fuel wall heat flux is directly related to the fuel temperature, since the wall heat flux is simply \(-k\nabla\cdot T\hat{n}\), where \(T\) is the fuel temperature. So, if the user is going to specify the wall heat flux, it would be good to ensure consistency with the initial condition specified on fuel temperature. 
...
\subsubsubsection{{\tt OpenMCTempScalarComponentIC}}
This class takes as input a set of expansion coefficients for fuel temperature and stores these coefficients in OpenMC data structures. There are two ways the user can utilize this class:

\begin{enumerate}
\item Provide the values of the expansion coefficients directly
\item Define a continuous function for the temperature, use a {\tt FunctionAux} auxkernel to apply this function to an auxvariable, integrate that auxvariable using the {\tt ZLDeconstruction} user object to compute a new scalar variable holding the initial expansion coefficients, and pass this second scalar variable to the initial condition class.
\end{enumerate}

This flexibility allows the user to specify either the coefficients directly or a continuous field for which the coefficients are determined internally. For both cases, a dummy variable must be provided of the correct order, though none of its entries are filled.

\subsubsubsection{{\tt WallHeatFluxFunctionIC}}
The continuous temperature field is also used to determine the continuous wall heat flux, which is deconstructed into a set of expansion coefficients for the wall heat flux. In this way, the initial conditions on wall heat flux and fuel temperature are consistent with one another.
