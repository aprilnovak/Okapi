...
This report is organized beginning with a description of the overall coupling methodology, followed by a discussion of the nuts-and-bolts requirements for coupling two codes in terms of the data transfer between the codes and executing non-MOOSE codes from within the MOOSE framework. Finally, the end of this document discusses the mechanics of coupling the three codes together in terms of the build configuration requirements.

This document serves as my working notes, so any lingering ``dangers'' that are not fully resolved are indicated in pink font.

\section{Overall Coupling Methodology}
This section discusses the overall coupling methodology that was pursued between OpenMC-BISON, BISON-Nek, and finally OpenMC-BISON-Nek. The immediate jump to the coupling of all three codes together was not performed because taking smaller steps often helps reveal difficulties that would otherwise be compounded and confusing when performing the full three-way coupling. When coupling two distinct projects, there are generally three ways to perform the coupling:
...
This section discusses the coupling between Nek and BISON, where Nek is wrapped as a Moose App (MOON) to facilitate this coupling. As discussed in future sections, the three-way coupling between OpenMC, Nek, and BISON will be achieved using OpenMC as the Master App, so to work towards this goal, the coupling between BISON and Nek is pursued using MOOSE as the Master App. MOON based it's coupling on using BISON as the Master App. The coupling discussed in this section is illustrated with example input files in {\tt buffalo/tests/bison-nek}. 

In a coupled solve, the following steps occur. We arbitrarily choose BISON to execute before MOON, so we choose to execute BISON on {\tt timestep\_begin}, and MOON on {\tt timestep\_end}. 

\begin{enumerate}
\item Initialize the MasterApp. The MasterApp does not perform any solves, but only exists to transfer data between its MultiApps.
\item Initialize BISON. Because BISON is a Moose ``animal,'' this simply calls the initialization method of the MOOSE executioner that we chose to use (for most cases, this will be {\tt Transient::init()}).
\item Initialize MOON. This calls the {\tt NekExecutioner::init()} method, which performs the regular initialization steps for any MOOSE App, followed by calling the {\tt nek\_init} subroutine found in the Nek source. This subroutine performs basic tasks such as reading Nek input files, initializing variables, generating the geometry, and applying initial conditions.
...
When coupling more than two applications using the MOOSE framework, there are several options for the coupling structure. In order to motivate the structure selected, an understanding of the limitations of the MOOSE coupling framework must be understood. The general coupling framework for a MOOSE simulation consists of a Master App and MultiApps. For each time step, unless sub-cycling is turned on (in which case one of the Apps may run at finer time steps than the others), all of the Apps will execute. The order in which the execution of the Apps occurs is controlled with the {\tt execute\_on} parameter, which can take several values, most commonly {\tt timestep\_begin} and {\tt timestep\_end}. The MultiApps that execute on the start of a time step and on the end of a time step bound the execution of the Master App. If there are more than two MultiApps to a Master App, then in order to ensure full exchange of information at each time step, a custom routine for transferring data {\it in time} (not a custom {\tt Transfer} class, to be discussed later) is required. This would require developing a custom {\tt MultiApp} class, which is where the execution in time is controlled.

To keep the coupling as simple as possible and to avoid the need to develop custom Multi Apps (which is more intensive than developing custom transfers, executioners, and time steppers), the coupling is chosen to be limited to two MultiApps per Master App. When selecting the Master App, the Master App can either perform a solve (and hence be one of Okapi, BISON, or MOON) or simply exist to transfer data between the Multi Apps (and hence be one of Okapi, BISON, MOON, PRONGHORN, RATTLESNAKE, etc. - the animal itself doesn't matter because it's only using capabilities in the MOOSE framework, not any physics-specific capabilities). If we restrict ourselves to only using two peer MultiApps per Master App, then the Master App must perform a physical solve. For a Master App that performs a physical solve, there are three options:

\begin{enumerate}
\item BISON is the Master App.
\item MOON is the Master App. 
\item Okapi is the Master App.
\end{enumerate}

Because multiple fuel pins can be simulated entirely in parallel (since a boundary coupling is employed with Nek), it would be inadvisable to have BISON be the Master App. This would restrict the coupling to a single fuel pin because there can only be a single Master App. A non-trivial amount of work with MPI could allow this configuration to work correctly, but it would be much easier for either Okapi or MOON to be the Master App. However, because MOON was initially developed with BISON as the Master App, the first efforts at coupling will attempt to reuse as much as possible from MOON, and use BISON as the Master App. This is discussed in the next section.

\begin{figure}[H]
\centering
\includegraphics[width=18.5cm]{OpenMC-BISON-Nek-complicated.png}
\end{figure}

All of the data transfer classes developed for this coupling follow the same naming convention of {\tt MultiApp<SubApp><MasterApp>Transfer}. Because MOOSE-wrapped Apps, rather than generic MOOSE ``animals,'' are being coupled, a distinction must be made on which App is the MultiApp and which is the Master App. For instance, if Okapi is the MasterApp, then {\tt MultiAppMooseOkapiTransfer} coordinates data transfer between Buffalo and Okapi when Okapi is the Master App. If Buffalo is the Master App, then the data transfer class {\tt MultiAppOkapiMooseTransfer} controls the data transfer. The reason this distinction is necessary, when the actions to be taken during the transfer are essentially the same, is that the MOOSE-wrapped Apps are not sufficiently generic to permit data transfer the same as if it were a Sub App or a Master App.

With Okapi as the Master App, either BISON can be run before MOON, or MOON run before BISON. If BISON executes on {\tt timestep\_begin}, and MOON on {\tt timestep\_end}, the sequence of events following initialization of all Apps is as follows, which is repeated for each Picard step:

\begin{enumerate}
\item Transfer from Okapi to BISON a kappa-fission distribution and a temperature BC.
\item Solve BISON.
\item Transfer from BISON to Okapi a temperature distribution and a heat flux BC.
\item Solve OpenMC.
\item Transfer from Okapi to MOON a heat flux BC (not updated by OpenMC from its most recent solve).
\item Solve Nek.
\item Transfer from MOON to Okapi a fluid density distribution and a temperature BC.
\end{enumerate}

This approach requires setting an initial condition on the:

\begin{itemize} 
\item kappa-fission distribution; the transfer of the kappa-fission distribution from Okapi to BISON requires developing some method for setting an initial condition on this parameter internally in OpenMC, since it is not a Moose variable that can be set from one of the Moose input files.
\item temperature BC; because this is a Moose variable, an estimate for this can simply be set in the master Moose input file.
\item fluid density distribution; because Okapi runs before Nek in this case, an initial condition must be set on this field - this can be done from within the OpenMC XML files.
\end{itemize}

For these three initial conditions, inconsistencies can easily be introduced - if the temperature BC grossly differs from the fluid density distribution specified, convergence might be slow.

If MOON executes on {\tt timestep\_begin} and BISON on {\tt timestep\_end}, the sequence of events following initialization of all Apps is as follows, which is repeated for each Picard step:

\begin{enumerate}
\item Transfer from Okapi to MOON a heat flux BC.
\item Solve Nek.
\item Transfer from MOON to Okapi a fluid density distribution and a temperature BC.
\item Solve OpenMC.
\item Transfer from Okapi to BISON a kappa-fission distribution and a temperature BC (not updated by OpenMC from its most recent solve).
\item Solve BISON.
\item Transfer from BISON to Okapi a temperature distribution and a heat flux BC.
\end{enumerate}
...
\begin{itemize}
\item heat flux BC; because this is a Moose variable, this can simply be set in the master input file.
\item temperature distribution; because OpenMC runs before BISON, an initial condition must be set on the temperature distribution. Because this distribution is internal to OpenMC, some method must be developed to set an initial condition on the temperature distribution.
\end{itemize}

For this setup, inconsistencies can also be introduced - the initial conditions on heat flux BC and temperature distribution can be conflicting.

\subsubsection{BISON Master App}
This section discusses the three-way coupling achieved between BISON, Okapi, and MOON, where BISON is the Master App. As discussed previously, this only works for a single fuel pin (one instance of BISON) because there cannot be multiple instances of a Master App without significant MPI ``glue'' code. 
...
\subsection{MOOSE (a.k.a. BISON) Data Transfer}
This section describes the capabilities introduced into the MOOSE source code that allows evaluation of polynomials, and construction/deconstruction of continuous fields. Because BISON is a Moose ``Animal,'' we can simply use MOOSE as a surrogate for BISON to avoid all of the complicated physics until the entire coupling framework is in place. So, some of these new classes developed would actually be implemented in BISON, and {\it not} in MOOSE. In this section, the only classes that would {\it not} be implemented in BISON are:

\begin{itemize}
\item MultiAppOkapiMooseTransfer (this isn't called by BISON - it's called by whatever is the MasterApp)
\end{itemize}
...
\color{magenta}
This function is available in Okapi, but should be moved to a MOOSE module for generality.
\color{black}
...
This user object determines an expansion coefficient given a field \(-k\nabla u(\theta, z)\cdot\hat{n}\) and the orders of the Fourier and Legendre expansions according to Eq. \eqref{eq:ScaledExpansionCoeff} for a cylindrical boundary. Note that this user object computes expansion coefficients for heat flux.

\color{magenta}
This user object should be modified to compute more than one expansion coefficient at a time to reduce bloat in the input file. This user object is available in MOON, but should be relocated to a MOOSE submodule under a name like {\tt FourierLegendreHeatFluxDeconstruction}, since it isn't specific to anything in Nek.
\color{black}
...
\color{magenta}
This function can be made more general by allowing the Legendre expansion to be in either \(x\), \(y\), or \(z\) instead of just \(z\). This function is available in MOON, but should be moved to a MOOSE module for generality.
\color{black}
...
The first approach is selected here because it requires less specifications than the second method (i.e. no assumptions are needed for values for \(\nu\) and \(E_f\)) and integration over a volume can easily be performed with a MOOSE postprocessor.
...
MOOSE controls all data transfers between the coupled external codes. This section describes the MOOSE {\tt Transfer} objects that were created to perform data transfer to/from OpenMC and to/from Nek. The OpenMC and Nek routines that are called from within the MOOSE transfer classes developed are discussed in Sections \ref{sec:OpenMCTransfer} and \ref{sec:NekTransfer}, respectively. 
...
This transfer object defines what routines are called to transfer data from MOOSE to Nek, and from Nek back to MOOSE. In order to be flexible such that the transfer can be used for both transfer to and from Nek, the {\tt to\_aux\_scalar} parameter will be a dummy parameter for transfer to Nek (because Nek does not perform any finite element solve in the MOOSE context), while {\tt source\_variable} will be a dummy variable for transfer from Nek for the same reason.
\\\\
{\it Transfer to Nek}\\
...
The {\tt userchk} subroutine is used to interrogate the solution, and is where the maximum Legendre and Fourier expansion orders are defined ({\tt nl\_max} and {\tt nf\_max}, respectively). 
...
! maximum Legendre and Fourier expansion orders
...
\subsubsubsection{{\tt init()}}
This method is called once at the very beginning of the simulation to perform tasks that are required for {\it all} Nek runs need later in the coupling simulation such as memory allocation, MPI setup, reading initial data, etc. The actions taken by this method are completely independent of any input information from MOOSE such as boundary conditions that are to be coupled during Picard iterations. This method calls the {\tt init()} method of the {\tt Transient} executioner, followed by the {\tt nek\_init} subroutine. This subroutine is defined in the core Nek source code, and performs the following actions:

\begin{enumerate}
\item If the communicator passed into the subroutine is not a valid communicator, then MPI is initialized. A duplicate communicator is created to avoid name collisions.
\item Read the {\tt SESSION.NAME}, {\tt SIZE}, and {\tt .rea} files and set values based on parameters in these files.
