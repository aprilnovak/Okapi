...
MultiAppMooseOkapiReactivityTransfer::MultiAppMooseOkapiReactivityTransfer(const InputParameters & parameters) :
...
{
  _console << "Beginning MultiAppMooseOkapiReactivityTransfer Transfer " <<
    name() << std::endl;
    ...
    case TO_MULTIAPP:
    {
     for (unsigned int I = 0; I < num_apps; ++I)
       if (_multi_app->hasLocalApp(I))
       {
         ...
         if (_dbg) _console << "Sending k_eff value of: " << OpenMC::get_keff() <<
           " from OpenMC to MOOSE!" << std::endl;

         std::vector<dof_id_type> & dof = to_vars->dofIndices();
         to_vars->sys().solution().set(dof[0], OpenMC::get_keff());
  ...
  _console << "Finished MultiAppMooseOkapiReactivityTransfer transfer " <<
    name() << std::endl;
}
